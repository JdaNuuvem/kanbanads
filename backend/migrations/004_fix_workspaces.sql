-- ============================================================================
-- 004_fix_workspaces — Aplica workspace se 003 não rodou (idempotente)
-- ============================================================================
...
DO $$ BEGIN
  ...
  ALTER TABLE activity ADD COLUMN IF NOT EXISTS workspace_id UUID;
END $$;

-- Aplica FK (se coluna foi adicionada agora e não tem FK)
DO $$ BEGIN
...
DO $$ BEGIN
  ALTER TABLE activity ADD CONSTRAINT fk_activity_workspace
...
SELECT u.id, u.name, u.role, u.color,
  COUNT(DISTINCT pa.product_id) FILTER (WHERE p.stage_id != 'morto' AND p.archived_at IS NULL) AS active_count,
  COUNT(DISTINCT pa.product_id) FILTER (WHERE p.stage_id IN ('rodando','escala')) AS running_count,
  COUNT(DISTINCT pa.product_id) FILTER (WHERE p.stage_id = 'rodando' AND p.entered_stage_at < now() - interval '7 days') AS stale_count,
...
SELECT s.id, s.title, s.position, s.color,
  COUNT(p.id) AS total,
...
BEGIN
  ...
  IF default_ws_id IS NULL THEN
    ...
    FOR user_record IN SELECT id FROM users WHERE active = true LOOP
      ...
      VALUES (default_ws_id, user_record.id,
        CASE WHEN user_record.id = admin_user_id THEN 'owner'
  ...
  -- Converte NULLs para o workspace default
